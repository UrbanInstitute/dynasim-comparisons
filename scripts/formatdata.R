# Hannah Recht, Urban Institute Income and Benefits Policy Center, 08-18-15
# Aaron Williams, Urban Institute Income and Benefits Policy Center, 6-29-2017
...
format <- function(dt) {
  ...
  # Purpose: Creates category and catlab variables. Takes variables for each
  #          percentile and income and combined them into sensible long data
  #          frames
  # Args:
  #   dt: wide dataframes with variables for each income X percentile
  # Output: Formatted data frames 
...
# Combine pc data frames for each reform into one data frame
...
# Gather and set aside the level observations
base <- pc %>%
  gather(key = "key", value = "value", netinc_1_pc:income_8_pc)

# Calculate $ change, combine it with the level values, and spread
pc <- pc %>%
  gather(key = "key", value = "value", netinc_2_pc:income_8_pc) %>%
  mutate(value_change = NA,
...
# Combine and transform equivalent-scaled data
...
# Gather and set aside the level observations
base <- eq %>%
  gather(key = "key", value = "value", netinc_1_eq:income_8_eq)

# Calculate $ change, combine it with the level values, and spread
eq <- eq %>%
  gather(key = "key", value = "value", netinc_2_eq:income_8_eq) %>%
  mutate(value_change = NA,
...
# Combine pc and eq into one data frame
...
write_csv(all, "data/allscenarios_new.csv", na = "")
